...
class TestCrypto:
    ...
    def test_fhenix_parallel(self):
        ...
        action = "test_fhenix"
        wallets_to_run = list(range(0, 50))  # run wallet 0-9
        max_workers = 1
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            for i in wallets_to_run:
                executor.submit(self.run_wallet, i, url, action)
    ...
    def test_hotstuff_trade(self):
        ...
        wallets_to_run = list(range(0, 11))
        max_workers = 1
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            for i in wallets_to_run:
                executor.submit(self.run_wallet, i, url, action)
    ...
    def test_prismax(self):
        for i in range(0, 1):
            ...
            if not page:
                ...
                continue  # 👈 go to next wallet
            try:
                page.goto("https://app.prismax.ai/")
                page.pause()
                #ProjectPage(page).test_prismax()
                print("✅ Completed wallet", i)
            except Exception as e:
    ...
    def test_robinhood(self):
        ...
        for i in range(0, 1):
            ...
            if not page:
                ...
                continue  # 👈 go to next wallet
            try:
                page.goto("https://faucet.testnet.chain.robinhood.com/")
                ProjectPage(page).test_robinhood()
                print("✅ Completed wallet", i)
            except Exception as e:
                print("❌ Wallet failed", i)
            finally:
